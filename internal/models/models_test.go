...
// TestRenderMarkdownMatchesCommittedDoc guards the committed /models command
// doc against drifting from the prose RenderMarkdown emits. The whole constant
// tail (from "## Switching the active model", i.e. everything after the
// account-specific model table) is compared, since updateModelsDoc regenerates
// the user's installed copy from RenderMarkdown — any drift here would silently
// overwrite the committed doc on the next catalog change.
func TestRenderMarkdownMatchesCommittedDoc(t *testing.T) {
	const marker = "## Switching the active model"

	rendered := RenderMarkdown(nil)
	i := strings.Index(rendered, marker)
	if i < 0 {
		t.Fatalf("RenderMarkdown output is missing %q", marker)
	}

	doc, err := os.ReadFile(filepath.Join("..", "..", "plugin", "commands", "models.md"))
	if err != nil {
		t.Fatalf("read committed models.md: %v", err)
	}
	j := strings.Index(string(doc), marker)
	if j < 0 {
		t.Fatalf("committed models.md is missing %q", marker)
	}

	renderedTail := rendered[i:]
	docTail := strings.ReplaceAll(string(doc[j:]), "\r\n", "\n")
	if renderedTail != docTail {
		t.Errorf("plugin/commands/models.md prose has drifted from RenderMarkdown; "+
			"update the committed doc to match.\n--- rendered ---\n%s\n--- committed ---\n%s",
			renderedTail, docTail)
	}
}
