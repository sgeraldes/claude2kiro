...
func (m *Model) loadSettings() {
	...
	m.settings[TabServer] = []Setting{
		...
		},
		{
			...
			Description: "Start server when opening the app",
			Type:        TypeToggle,
			...
			ExtendedHelp: ExtendedHelp{
				...
				Sensitive:        false,
				DetailedDesc:     "When enabled, the proxy server starts automatically when the TUI launches. The server runs in the background regardless of which view is shown first.",
			},
		},
		{
	...
	m.settings[TabAdvanced] = []Setting{
		...
		},
		{
			...
			Label:       "Debug Mode",
			Description: "Write debug files for every request",
			Type:        TypeToggle,
			Value:       boolToString(m.config.Advanced.DebugMode),
			ExtendedHelp: ExtendedHelp{
				DefaultValue:     "false",
				RecommendedValue: "false",
				Sensitive:        false,
