...
// LogViewerModel is single-threaded state owned by the Bubble Tea event loop.
// Do not access or mutate it directly from background goroutines.
// Use tea.Program.Send() to deliver external events into the TUI loop.
