...
const (
	...
	ActionServer
	ActionDashboard
	ActionLaunchClaude // Launch Claude Code in new terminal connected to proxy
	ActionRefreshToken
...
func (m *Model) rebuildMenuItems() {
	...
	// 4. Configure Claude - disabled, `run` and `remote` handle config per-session
...
func (m Model) View() string {
	...
	// Claude Code status (run/remote handle config per-session)
	if m.claudeConfig.FileExists {
		statusLines = append(statusLines,
			labelStyle.Render("Claude Code: ")+okStyle.Render("✓ Use 'run' or 'remote' to launch"))
	} else {
		statusLines = append(statusLines,
